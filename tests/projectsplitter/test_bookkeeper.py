...
def test_update(datadir):
    ...
    data = {
        ...
        os.path.join(datadir, "removed.pdf"): False,
    }
    ...
    assert bk._data[os.path.join(datadir, "foo.pdf")] is False

    assert bk._data[os.path.join(datadir, "foo.xml")]
    assert bk._data[os.path.join(datadir, "foo.csv")]
    assert bk._data[os.path.join(datadir, "d1/bar.pdf")] is False
...
def test_consumed(tmp_path):
    "Test marking a file as consumed."
    bk = BookKeeper(tmp_path)
    the_file = os.path.join(tmp_path, "foo.xml")
    bk._data[the_file] = False

    bk.consumed(the_file)
    assert bk._data[the_file]
