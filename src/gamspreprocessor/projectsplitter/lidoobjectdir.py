...
class ResourceSet:
    ...
    def get_new_url(self) -> str:
        """Return the new (relative) URL for the resource."""
    ...
    def from_element(
        ...
        if resourceid_element is not None:
            resource_id = resourceid_element.text or ""
            resource_type = resourceid_element.attrib.get(
                f"{{{namespaces['lido']}}}type", ""
            )
        else:
            resource_id = ""
            resource_type = ""

        if resourcelink_element is not None:
            link_format = resourcelink_element.attrib.get(
                f"{{{namespaces['lido']}}}formatResource", ""
            )
            link_url = resourcelink_element.text or ""
        else:
            link_format = ""
            link_url = ""
